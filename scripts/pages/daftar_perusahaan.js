(function () {
  ...
  const cancelButton = document.getElementById('cancel-company-btn');
  ...
  function writeData(rows) {
    localStorage.setItem(COMPANY_KEY, JSON.stringify(rows));
  }
  ...
  window.addEventListener('storage', function (event) {
    if (!event || (event.key !== COMPANY_KEY && event.key !== null)) return;
    renderRows();
  });
  ...
  window.addEventListener('aios:cloud-sync', function (event) {
    const changedKeys = event && event.detail && Array.isArray(event.detail.changedKeys)
      ...
      : [];
    if (changedKeys.length > 0 && changedKeys.indexOf(COMPANY_KEY) < 0) return;
    renderRows();
  });
